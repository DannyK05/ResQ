        ...
        <main>
            ...
            <div className="dialogue">
                {dialogue.map((item, index) => (
                    <div key={index} className={`flex ${item.speaker === 'bot' ? 'flex-row' : 'flex-row-reverse'}  justify-between align-center my-5 px-[20px] ${item.speaker === 'bot' ? 'bot-message' : 'user-message'}`}>
                        {item.speaker === 'bot' ? <img src={ResQBot}  alt="avatar" />: <ion-icon name="person"></ion-icon>}
                        <span className={`${item.speaker === 'bot' ? 'bg-blue text-white' : 'bg-white text-blue'} p-[5px] w-5/6 shadow shadow-black rounded-[6px]`}>
                            <p >{item.message}</p>
                        </span>
                    </div>
                ))}
