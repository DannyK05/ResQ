...
const Map = ({ apiKey }) => {
  ...
  useEffect(() => {
    if (
      userLocation.userLatitude !== null &&
      userLocation.userLongitude !== null
    ) {
  ...
  const fetchNearbyHospitals = async (lat, lng) => {
    try {
      ...
      const response = await axios.get(
        "https://resq-google-api-proxy-server-1.onrender.com/fetch-nearby-hospitals",
        {
          params: {
            lat: lat,
            lng: lng,
  ...
  if (
    ...
    return (
      <div className="flex flex-col justify-center items-center color-blue">
        <h1>Loading...</h1>
      </div>
    );
  }
