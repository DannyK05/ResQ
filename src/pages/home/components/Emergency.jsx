...
const EmergencyTab = ({ isVisible }) => {
  ...
  const fetchNearbyHospitals = async () => {
    try {
      ...
      const response = await fetch(
        `https://resq-google-api-proxy-server-1.onrender.com/maps/api/place/nearbysearch/json?location=${userLocation.latitude},${userLocation.longitude}&radius=10000&type=hospital&key=AIzaSyA-eimkkqp9OSHxHlKuScXbyz9Cr-dgqf0`,
        {
          method: "GET",
          headers: {
      ...
      if (hospitalData) {
        if (hospitalData.length > 0) {
          ...
          let closestDistance = calculateDistance(
            userLocation,
            closestHospital.geometry.location
          );
  ...
  const callClosestHospital = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://resq-google-api-proxy-server-1.onrender.com/call-closest-hospital?userLocation=${userLocation.latitude},${userLocation.longitude}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
