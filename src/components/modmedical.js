...
const ModMedical = ({visibility, Close}) => {
    return(
        <section className={`${visibility ? 'absolute' : 'hidden'} w-[100%] bg-white py-[10px] px-[15px] rounded-[20px] z-10 shadow shadow-black top-[20px]`}>           
           <h1>Manage your medical info <ion-icon onClick={Close} size = "large" name="close-circle"></ion-icon></h1>
            <form className="overflow-scroll">
                ...
                <input name="allergy" className="ml-[8px]" type ="radio" value="Yes" /> <label>Yes</label>
                <input name = "allergy" className="ml-[8px]" type ="radio" value="No"/><label>No</label>
                <br/>
                <label>If yes please specify</label><input className="border-2 border-[#E7DDDD] p-[5px] hover:border-[#2592F6] m-[10px] w-[80%]" name='allergen' type= "text"/>
                <br/>
